...
function testResizeGeometraThreeWebGLWithSceneBasicsViewFromPlainViewSizingMatchesExplicitResize() {
  const log = []
  const renderer = {
    setPixelRatio(pr) {
      log.push(['setPixelRatio', pr])
  ...
  const bundle = { renderer, camera }
  const sizing = toPlainGeometraThreeViewSizingState(800, 400, 3, 2)
  resizeGeometraThreeWebGLWithSceneBasicsViewFromPlainViewSizing(bundle, sizing)
  const fromPlain = [...log]
  log.length = 0
  resizeGeometraThreeWebGLWithSceneBasicsView(bundle, 800, 400, 3, 2)
  assert.deepEqual(fromPlain, log)

  const snap = toPlainGeometraThreeHostSnapshot(640, 360, 2, 2)
  log.length = 0
  resizeGeometraThreeWebGLWithSceneBasicsViewFromPlainViewSizing(bundle, snap)
  const fromSnap = [...log]
  log.length = 0
  resizeGeometraThreeWebGLWithSceneBasicsView(bundle, 640, 360, 2, 2)
  assert.deepEqual(fromSnap, log)
}
...
function testResizeTickGeometraThreeWebGLWithSceneBasicsFromPlainViewSizingParity() {
  const log = []
  const clock = new THREE.Clock()
  const scene = {}
  const camera = {
  ...
  const sizing = toPlainGeometraThreeViewSizingState(800, 400, 2, 2)

  assert.equal(
    resizeTickGeometraThreeWebGLWithSceneBasicsFromPlainViewSizing(bundle, sizing, () => {
      log.push('onFrame')
    }),
    true,
  )
  ...
  log.length = 0
  assert.equal(
    resizeTickGeometraThreeWebGLWithSceneBasicsFromPlainViewSizing(bundle, sizing, () => false),
    false,
  )
  assert.deepEqual(log, [
    ['setPixelRatio', 2],
  ...
  log.length = 0
  assert.equal(resizeTickGeometraThreeWebGLWithSceneBasicsFromPlainViewSizing(bundle, sizing), true)
  const combinedLog = [...log]
  log.length = 0
  resizeGeometraThreeWebGLWithSceneBasicsViewFromPlainViewSizing(bundle, sizing)
  const resizeLog = [...log]
